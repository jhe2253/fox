...
public class PlatformSpawner : MonoBehaviour
{
    ...
    private Vector2 poolPlsition = new Vector2(0, -25);
    ...
    void Start()
    {
        ...
        {
            // platformPrefad을 원본으로 새 발판을 poolPosition 위치에 복제 생성
            // 생선된 발판을 platform 배열에 할당
            platforms[i] = Instantiate(platformPrefab, poolPlsition, Quaternion.identity);
        }
        //마지막 배치 시점 초기화
        lastSpawnTime = 0f;
    ...
    void Update()
    {
        ...
        if (Time.time >= lastSpawnTime + timeBetSpawn)
        {
            ...
            // 배치할 위치의 높이를 yMin과 yMax 사이에서 램덤 설정
